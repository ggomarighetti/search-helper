...
final class SearchPageableGuard {
    ...
    public SearchPageableGuard(SearchPolicy policy) {
    ...
    public Pageable pageable(Pageable pageable, SearchDefinition<?> definition) {
        ...
        return pageable(
                pageable,
                definition,
                new SearchProtectionContext(effectivePolicy, SearchCompilationMode.PAGE));
    }
    ...
    private Sort sort(
        ...
        for (Sort.Order order : sourceOrders) {
            ...
            SearchField<?> field = definition.field(order.getProperty()).orElseThrow(this::sortRulesForbidden);
    ...
    private SearchPageableValidationException pageRulesForbidden(List<RuleViolation> violations) {
        return new SearchPageableValidationException(
                SearchPageableValidationException.PAGE_RULES_FORBIDDEN,
                "Pageable page or size failed validation with %d violation(s).".formatted(violations.size()),
                violations);
    }
